...
- 着実に習慣化したい初学者
  - 幅広い分野（例：Ruby、Ruby on Rails、HTML・CSS、JavaScript、SQL、Git、Dockerなど）を一度に学ぶのは挫折の原因になりがちです。
...
- 学習ジャンル設定
  - 初回投稿前に、自分が取り組むジャンル（例：Ruby、JavaScriptなど）を設定できます。
  途中でジャンル変更や追加（最大3つまで）も可能です。

- 学習記録投稿
  - ジャンルごとに「学習内容」「画像」「学んだこと」を記録できます。
    1ジャンルにつき21日間継続投稿を目指します。
    投稿後、AIが学習内容を分析し、自動で練習問題を生成。すぐ挑戦可能です。

- ユーザー交流
  - 投稿に「いいね」や「コメント」が可能です。
  - コメントすると相手に通知（LINEログイン時はLINE通知可）も可能です。
  - 自分の記録はTwitterでシェア可能です。

- 学習の可視化
  - Contributions Graph：投稿日ごとに色がつく。学習時間が長いほど濃く表示します。
    学習カレンダー：開始時間・終了時間を記録が表示されます。
    通知機能：設定時間の開始・終了時にアラート（LINE連携可)が来ます。

- 情報・参考機能
  - ジャンル詳細ページでWikipediaの概要表示されます。
    グラフで学習継続状況を確認できます。
    他ユーザーのジャンル一覧から学習テーマを探せます。

- ゲーム要素
  - 継続日数に応じたバッジ獲得できます。
  - 毎日投稿でランキング戦に参加（同率順位あり）できます。
   

## ▪️ ユーザーの獲得・宣伝方法

　RUNTEQの学習仲間やTwitterでのshareによるユーザーの獲得します。

## ▪️ サービスの差別化ポイント・推しポイント

- 21日習慣化サイクル
  - 習慣化に必要と言われる3週間ごとに新しいジャンルを追加可能です。
    一度に手を広げすぎず、着実にスキルを積み上げられます。
- 学習の徹底可視化
  - Contributions Graphやカレンダーなど、多様なグラフで学習状況を視覚化します。
  - ユーザーインタビューでも「学習の見える化」が高評価でした。
- 楽しく続けられるゲーム要素
  - バッジ獲得、ランキング戦、AI生成の問題挑戦など、学習をエンタメ化。

- 多彩な通知機能でモチベ維持
  - 自分だけでなく他ユーザーの学習開始も通知（ブラウザ通知）されます。
    仲間と一緒に学んでいる感覚を醸成します。
...
| カテゴリ       | 技術                                                                     |
| -------------- | ------------------------------------------------------------------------ |
| フロントエンド | Rails 7.2.2 / Tailwind CSS/ Hotwire                                       |
